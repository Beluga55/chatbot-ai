async function retrieveInformation() {
  ...
  if (navList) {
    const clickedLink = event.currentTarget.querySelector("p");

    if (clickedLink.textContent === "Users") {
      const response = await fetch(
        "https://chatbot-rreu.onrender.com/retrieveUsersAdmin",
        {
          method: "GET",
          headers: {
      ...
      const response = await fetch(
        "https://chatbot-rreu.onrender.com/retrieveChatHistoryAdmin",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
