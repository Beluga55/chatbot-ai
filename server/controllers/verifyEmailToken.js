...
export const verifyEmailToken = async (req, res) => {
  ...
  try {
    ...
    res.send(`
      <script>
        window.location.href = 'http://localhost:5173/user.html?showNotyf=true';
      </script>
    `);
  } catch (error) {
...
export const verifySignupEmailToken = async (req, res) => {
  ...
  try {
    ...
    if (username !== null) {
      return res.send(`
        <script>
          window.location.href = 'http://localhost:5173/user.html?showNotyf=true&linkClicked=true';
        </script>
      `);
    } else {
      ...
      return res.send(`
        <script>
          window.location.href = 'http://localhost:5173/login.html?showNotyf=true&linkClicked=true';
        </script>
      `);
    }
  } catch (error) {
